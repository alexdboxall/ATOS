...
int thread_fork(void (*initial_address)(void*), void* argument) {
    panic("not implemented");

    if ((size_t) initial_address < ARCH_USER_AREA_BASE || (size_t) initial_address >= ARCH_USER_AREA_LIMIT) {
        panic("thread_fork: fork cannot resume on kernel stack, and therefore initial_address be in userland");
    }
    ...
    */
    *thr = *current_cpu->current_thread;
    kprintf("B\n");
...
#ifndef NDEBUG
    /*
    * This is very slow, and so is only enabled in debug mode.
    */
